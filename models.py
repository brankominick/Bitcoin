...
#return best option
    ...
    #print 'accs[0]',len(accs[0])
    #10 sets of data each with 100 accuracies, so get list with each one

    for i in range(len(accuracies)):  #do t paired t test to compare with chosen
        #if ((ind-1==i) or (ind-1 not in range(len(accs)))):
        #    pass
        #else:
        indList = [accs[n][ind-1] for n in range(len(accs))]
        compList = [accs[n][i] for n in range(len(accs))]
        #print 'indlist',indList
        #print 'complist',compList
        print ind,i+1#, accs[ind-1],accs[i]
        #print ttest_rel(accs[ind-1],accs[i])
        print ttest_rel(indList,compList)
